/**
 * Faculty — AI Question Studio mock API (Phase 7).
...
/* ---------------- Summary + metrics ---------------- */
mockRoute('get', '/faculty/question-studio', () => {
...
/* ---------------- Source Library ---------------- */
mockRoute('get', '/faculty/question-studio/sources', ({ params }) => {
...
mockRoute('get', '/faculty/question-studio/sources/:id', ({ params }) => {
...
/* ---------------- Source analysis ---------------- */
mockRoute('post', '/faculty/question-studio/sources/:id/analyze', ({ params }) => {
  const source = sourceOf(params.id)
  if (!source) {
    const err = new Error('Source not found.')
    err.response = { status: 404, data: { message: err.message } }
...
/* ---------------- Simulated upload ---------------- */
mockRoute('post', '/faculty/question-studio/sources/upload', ({ body }) => {
...
/* ---------------- Generation ---------------- */
mockRoute('post', '/faculty/question-studio/generate', ({ body }) => {
...
/* ---------------- Sessions ---------------- */
mockRoute('get', '/faculty/question-studio/sessions', () => {
  const sessions = readSessions()
  return {
...
mockRoute('post', '/faculty/question-studio/sessions/:id/questions/:qid/edit', ({ params, body }) => {
  const sessions = readSessions()
  const session = sessions.find((s) => s.studioSessionId === params.id)
...
mockRoute('post', '/faculty/question-studio/sessions/:id/questions/:qid/delete', ({ params }) => {
  const sessions = readSessions()
  const session = sessions.find((s) => s.studioSessionId === params.id)
  if (!session) {
    const err = new Error('Session not found.')
...
mockRoute('post', '/faculty/question-studio/sessions/:id/questions/:qid/approve', ({ params }) => {
  const sessions = readSessions()
  const session = sessions.find((s) => s.studioSessionId === params.id)
  const q = session?.questions?.find((x) => x.questionId === params.qid)
  if (!session || !q) {
...
mockRoute('post', '/faculty/question-studio/sessions/:id/questions/:qid/reject', ({ params }) => {
  const sessions = readSessions()
  const session = sessions.find((s) => s.studioSessionId === params.id)
  const q = session?.questions?.find((x) => x.questionId === params.qid)
  if (!session || !q) {
