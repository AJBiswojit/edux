...
function AITutor({ embedded = false, promptSignal = null }) {
  ...
  const send = async (text) => {
    ...
    setLoading(true)
    try {
      const { reply } = await ask({ text: trimmed, threadId: activeId ?? 'new' })
      setMessages((m) => [...m, { id: `a_${Date.now()}`, role: 'assistant', text: reply, time: new Date().toISOString() }])
    } catch {
      /* Phase 27.1 (Part 22): never show "Assistant Offline". Fall back to the
         deterministic contextual reply engine so the experience stays alive. */
  ...
  const totalSessions = statsData?.totalSessions ?? 86
    ...
    <div className={`flex min-h-[520px] flex-col gap-5 lg:flex-row ${embedded ? 'h-[calc(100vh-16rem)]' : 'h-[calc(100vh-7rem)]'}`}>
      ...
      <aside className="hidden w-72 shrink-0 flex-col rounded-3xl border border-slate-200/70 bg-white p-4 shadow-card lg:flex dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="mt-3 rounded-2xl bg-gradient-to-br from-indigo-600/10 to-teal-500/10 p-3.5 ring-1 ring-indigo-500/15">
          ...
          <div className="mt-2 grid grid-cols-3 gap-1.5 text-center">
            <div>
              <p className="text-sm font-bold text-slate-800 dark:text-white">14</p>
              <p className="text-[9px] font-medium text-slate-400">Sessions</p>
            </div>
            <div>
              <p className="text-sm font-bold text-slate-800 dark:text-white">4.9</p>
              <p className="text-[9px] font-medium text-slate-400">Rating</p>
            </div>
            <div>
              <p className="text-sm font-bold text-slate-800 dark:text-white">6h</p>
      ...
      <Card className="flex min-w-0 flex-1 flex-col overflow-hidden p-0">
        ...
        <div ref={scrollRef} className="flex-1 space-y-5 overflow-y-auto scrollbar-thin px-4 py-6 sm:px-6">
          {messages.length === 0 && !loading && (
            <div className="flex h-full flex-col items-center justify-center text-center">
              ...
              <div className="mt-6 grid w-full max-w-lg grid-cols-1 gap-2 sm:grid-cols-2">
                {['Explain recursion with an analogy', 'Quiz me on TCP congestion control', 'Compare Dijkstra vs Bellman-Ford', 'Plan my revision for midsems'].map((q) => (
          ...
          {messages.map((m) => (
            <ChatMessage key={m.id} message={m} senderName="Aarav" />
          ))}
          {loading && <TypingDots />}
        </div>
