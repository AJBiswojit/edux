        ...
        {!searching && searchQ.length <= 2 && (
          <div className="flex h-full flex-col items-center justify-center text-center">
            ...
            <p className="mt-1 max-w-[220px] text-xs leading-relaxed text-slate-400">
              Try “TCP congestion control” or “transformer attention” — every result comes with citations.
            </p>
          </div>
        )}
      </div>
...
function AICopilot() {
    ...
    setMessages([
      {
        id: 'intro',
        role: 'assistant',
        text: 'Copilot workspace ready 🚀 — this is your **always-on AI workspace**. Unlike the Tutor (subject learning), I handle **everything else**: writing, research, summarising, planning.\n\nTry a prompt like:\n- “Write a 200-word summary of my ML project for my portfolio”\n- “Research: how do transformers handle long context?”\n- “Draft an email to Dr. Krishnan about the assignment extension”',
      },
    ])
  }, [])
  ...
  const send = async (text) => {
    ...
    try {
      ...
      setMessages((m) => [...m, { id: `a_${Date.now()}`, role: 'assistant', text: fallback, time: new Date().toISOString() }])
    ...
    <div className="grid h-[calc(100vh-7rem)] min-h-[560px] gap-5 lg:grid-cols-[1fr_340px]">
      ...
      <Card className="flex min-w-0 flex-col overflow-hidden p-0">
        ...
        <div ref={scrollRef} className="flex-1 space-y-5 overflow-y-auto scrollbar-thin px-4 py-6 sm:px-6">
          {messages.map((m) => <ChatMessage key={m.id} message={m} senderName="Aarav" />)}
          {loading && <TypingDots />}
        </div>
